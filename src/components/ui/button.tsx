...
const buttonVariants = cva('inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0', {
    variants: {
        variant: {
            ...
            outline: 'border border-input bg-background shadow-sm hover:bg-accent hover:text-accent-foreground hover:brightness-90',
            secondary: 'bg-secondary text-secondary-foreground shadow-sm hover:brightness-90',
            ghost: 'hover:bg-accent hover:text-accent-foreground hover:brightness-90',
            link: 'text-primary underline-offset-4 hover:underline',
            yellow: 'bg-yellow text-white disabled:bg-gray  text-base font-semibold shadow-none',
        },
        size: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return (
        <Comp
            className={cn(buttonVariants({ variant, size, className }), 'transition-transform duration-300 active:scale-95 hover:scale-105')}
            ref={ref}
            {...props}
        />
    );
