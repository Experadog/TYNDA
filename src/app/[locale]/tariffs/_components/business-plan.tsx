...
import { TbCoins, TbRosetteDiscountFilled } from 'react-icons/tb';
...
interface IProps {
  businessPlanViewModel: ViewModel['Tariffs']['businessPlan']
}

const BusinessPlan: FC<IProps> = ({ businessPlanViewModel }) => {
  ...
  return (
    <>
      <div className='flex flex-col items-center gap-10 mt-20 lg:mt-16'>
        <h3 className='text-5xl lg:text-4xl font-semibold text-center'>
          {businessPlanViewModel.title1}
          {" "}
          <span className='text-yellow'>{businessPlanViewModel.title2}</span>
          {" "}
          {businessPlanViewModel.title3}
        </h3>
      </div>
      <div className='mt-[40px] h-auto bg-yellow bg-no-repeat flex gap-[46px] lg:gap-[30px] justify-between lg:flex-col lg:items-center p-5 pl-[50px] lg:px-[17px] lg:py-[34px] rounded-[35px]'>
        <div className='flex items-center gap-9 lg:flex-col-reverse'>
          <div className='bg-background_1 flex flex-col p-[15px] rounded-[25px] max-w-[356px]'>
            {features.map((item, index) => (
              <div key={index} className='flex items-center gap-[10px] py-2'>
                ...
                <p className='text-foreground_1 text-base font-medium'>{businessPlanViewModel.bonuses[index]}</p>
              </div>
            ))}
          </div>
          <div className='w-[505px] h-[453px] z-10 lg:hidden'>
            <Image
              priority
              className='w-full'
              src='/tariffs/businessCard.webp'
              alt='card image'
              width={505}
              height={453}
            />
          </div>
          <div className='flex flex-col items-start lg:items-center gap-[195px] lg:gap-[60px]'>
            <div className='flex flex-col gap-[15px] max-w-[272px]'>
              <h3 className='text-3xl font-semibold text-white'>{businessPlanViewModel.titleBlock}</h3>
              <p className='text-xl font-medium text-white'>
                {businessPlanViewModel.description}
              </p>
            </div>
            ...
            <div className='flex flex-col gap-5 lg:gap-14 max-w-[352px]'>
              ...
              <div className='flex flex-col gap-[10px] px-[15px]'>
                <p className='text-sm font-medium text-white opacity-90 text-center max-w-[352px]'>
                  {businessPlanViewModel.autoRenewed}
                </p>
              </div>
            </div>
          </div>
        ...
        <div
          ...
          <div
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-lg text-foreground_1 font-semibold mb-4">{businessPlanViewModel.modalText}</p>
              <button
                onClick={() => setModalOpen(false)}
