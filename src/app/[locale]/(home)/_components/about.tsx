...
interface IProps {
	aboutViewModel: ViewModel['Home']['about'];
}

const About: FC<IProps> = ({ aboutViewModel }) => {
	return (
		<div className="mt-[120px] lg:mt-[60px] flex flex-col gap-[60px] lg:gap-[30px] mb-[200px] lg:mb-[60px]">
				...
				<Translate direction="right" distance={100} animateOnce={false}>
					<div className="flex flex-col gap-[20px] lg:items-start lg:justify-center">
						<h3 className="uppercase font-semibold text-lg opacity-70">
							{aboutViewModel.title}
						</h3>
						<h2 className="uppercase font-medium text-[34px] max-w-[734px]">
							{aboutViewModel.title1}{' '}
							<span className="text-[var(--yellow)]">{aboutViewModel.title2} </span>
							{aboutViewModel.title3}{' '}
							<span className="text-[var(--yellow)]">{aboutViewModel.title4}</span>
						</h2>
						<p className="text-lg opacity-80 font-normal max-w-[510px] mt-[20px]">
							{aboutViewModel.description1}
						</p>
						<p className="text-lg opacity-80 font-normal max-w-[510px]">
							{aboutViewModel.description2}
						</p>
					</div>
				</Translate>
			...
			<Translate direction="down" distance={50} animateOnce={false}>
				<div className="flex justify-between gap-5 lg:flex-col-reverse md:justify-center">
					...
					<div className="flex flex-wrap lg:flex-col items-center lg:items-start justify-center gap-4">
						<div className="flex flex-col gap-[10px]">
							...
							<p className="font-normal text-base">{aboutViewModel.stats1}</p>
						</div>
						<div className="flex flex-col gap-[10px]">
							<p className="font-bold text-6xl uppercase text-yellow numeric">50+</p>
							<p className="font-normal text-base">{aboutViewModel.stats2}</p>
						</div>
						<div className="flex flex-col gap-[10px]">
							<p className="font-bold text-6xl uppercase text-yellow numeric">
								3500+
							</p>
							<p className="font-normal text-base">{aboutViewModel.stats3}</p>
						</div>
					</div>
				</div>
			</Translate>
